function varargout = rectDE(varargin)
% DE M-file for DE.fig
% Theoretical SPDs -- RECTANGULAR
% See also: GUIDE, GUIDATA, GUIHANDLES

% Edit the above text to modify the response to help DE

% Last Modified by GUIDE v2.5 15-Apr-2013 15:08:32

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @rectDE_OpeningFcn, ...
                   'gui_OutputFcn',  @rectDE_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function rectDE_OpeningFcn(hObject, eventdata, handles, varargin)
...
check_value = get(handles.checkbox4, 'Value');
    ...
    saveString = strcat('Rectangular',' N=',num2str(N),' Gen=',num2str(gen),...
    ' P=',num2str(P),'');
...
% --- those with scores higher than the original scores are added
        ...
        for i = 1:P
          temp1= zeros(N, 81);
          for j = 1:N
            start = round(temp(i, 2*N+j));
            stop = round(start + temp(i,N+j)-1); 
            if stop > 81 stop = 81; end
            for k = start:stop 
                temp1(j,k) = temp(i,j); 
            end
...
% ---------- Finish: score > fitness OR all geneerations checked ----------
    ...
    temp1= zeros(N, 81);
    for j = 1:N
        start = round(bestoverall(2*N+j));
        stop = round(start + bestoverall(N+j)-1);  
        if stop > 81 stop = 81; end
        for k = start:stop 
           temp1(j,k) = bestoverall(j); 
        end
    end
...
function varargout = rectDE_OutputFcn(hObject, eventdata, handles) 
