function varargout = triDE(varargin)
% DE M-file for DE.fig
% Theoretical SPDs -- TRIANGULAR
% See also: GUIDE, GUIDATA, GUIHANDLES

% Edit the above text to modify the response to help DE

% Last Modified by GUIDE v2.5 15-Apr-2013 15:19:23

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @triDE_OpeningFcn, ...
                   'gui_OutputFcn',  @triDE_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function triDE_OpeningFcn(hObject, eventdata, handles, varargin)
...
mydata = getappdata(handles.figure1,'mydata');
    ...
    CR = str2num(get(handles.editCR, 'String')); % crossover constant CR
        ...
        saveString = strcat('Triangular',' N=',num2str(N),' Gen=',num2str(gen),...
    ' P=',num2str(P),'');
...
if ~isequal(filename,0) | ~isequal(pathname,0)
    ...
    % constants
    lambdaMinNorm = 1; % --> 380nm
    lambdaMaxNorm = 81; % --> 780nm
    Imin = 0.05;
    Imax = 1;
    FWHMMin = 2; % * 5 --> 10nm
    FWHMMax = 3; % --> 15nm
    % save the parameters of this search
    fprintf(fid, 'Gen: %u\tP: %u\t', gen, P);
    fprintf(fid, 'F: %4.2f\tCR: %5.2f\tN: %u\n',F, CR, N);
    % calculate wavelenght regions, depends on # LEDs
    % N = # LEDs
    dlambda = ( lambdaMaxNorm - lambdaMinNorm ) / N;
    %lambdas = zeros(N,1);
    lambdas(1) = lambdaMinNorm;
    for i=2:N
        lambdas(i) = round( lambdas(i-1) + dlambda );
    end
    lambdas(N+1) = lambdaMaxNorm;  
...
% ----------------- Generate initial population ---------------------------
    ...
    d = FWHMMax - FWHMMin;
    pop(:,N+1:2*N) = round( FWHMMin + d*rand(P, N) ); % random widths
    % wevelenghts
    d = lambdas(2)-lambdas(1);
    for i=1:N
        pop(:,2*N+i)=  lambdas(i) + round(d * rand(P, 1));  % random lamdas
    end
    ...
    spd = zeros((lambdaMaxNorm-lambdaMinNorm)+ 1, P);% spds of a population
    ...
    for i = 1:P
       temp1= zeros(N, 81);
       for j = 1:N
            start = pop(i, 2*N+j)-pop(i, N+j);
            stop = pop(i, 2*N+j)+pop(i, N+j);
            if start < 1 start = 1; end
            if start > 81 stop = 81; end
            if stop > 81 stop = 81; end
            if pop(i, 2*N + j)>81 pop(i, 2*N + j)=81; end
            a = pop(i,j) / pop(i, N+j);
            b = a * pop(i, 2*N+j);
            for k = start:pop(i, 2*N + j); 
                temp1(j,k) = temp1(j,k) + a * k - b + pop(i,j);     %temp + I/d*x - I*lambda/d + I
            end
            for k = pop(i, 2*N + j)+1:stop; 
                temp1(j,k) = temp1(j,k) - a * k + b + pop(i,j);     % temp -I/d*x + I*lambda/d + I
            end
       end
       for k = 1:81 
           for l = 1: N
               spd(k,i)= spd(k,i) + temp1(l,k);
           end
       end
    end
...
%----------Mutate and crossover - create a new population------------------
        ...
        temp(:,N+1:3*N) = round( temp(:,N+1:3*N) ); % random widths
        ...
        for i = 1:P
            ...
            for j = 2*N+1:3*N
                if temp(i, j) > lambdaMaxNorm
                    temp(i, j) = lambdaMaxNorm;
                else if temp(i, j) < lambdaMinNorm
                    temp(i, j) = lambdaMinNorm;
                    end
                end
            end
        end         
...
% --- those with scores higher than the original scores are added
        ...
        for i = 1:P
          temp1= zeros(N, 81);
          for j = 1:N
            start = temp(i, 2*N+j)-temp(i, N+j);
            stop = temp(i, 2*N+j)+temp(i, N+j);
            if start < 1 start = 1; end
            if start > 81 stop = 81; end
            if temp(i, 2*N + j)>81 temp(i, 2*N + j)=81; end
            if stop > 81 stop = 81; end
            a = temp(i,j) / temp(i, N+j);
            b = a * temp(i, 2*N+j);
            for k = start:round(temp(i, 2*N + j)); 
                temp1(j,k) = temp1(j,k) + a * k - b + temp(i,j);     %temp + I/d*x - I*lambda/d + I
            end
            for k = temp(i, 2*N + j)+1:stop; 
                temp1(j,k) = temp1(j,k) - a * k + b + temp(i,j);     % temp -I/d*x + I*lambda/d + I
            end
          end
          for k = 1:81 
...
% ---------- Finish: score > fitness OR all geneerations checked ----------
    ...
    spd = zeros((lambdaMaxNorm-lambdaMinNorm)+ 1, 1);% spd this
    temp1= zeros(N, 81);
    for j = 1:N
       start = bestoverall(2*N+j)-bestoverall(N+j);
       stop = bestoverall(2*N+j)+bestoverall(N+j);
       if start < 1 start = 1; end
       if start > 81 stop = 81; end
       if bestoverall(2*N + j)>81 bestoverall(2*N + j)=81; end
       if stop > 81 stop = 81; end
       a = bestoverall(j) / bestoverall(N+j);
       b = a * bestoverall(2*N+j);
       for k = start:bestoverall(2*N + j); 
           temp1(j,k) = temp1(j,k) + a * k - b + bestoverall(j);     %temp + I/d*x - I*lambda/d + I
       end
       for k = bestoverall(2*N + j)+1:stop; 
           temp1(j,k) = temp1(j,k) - a * k + b + bestoverall(j);     % temp -I/d*x + I*lambda/d + I
       end
    end
    for k = 1:81 
       for l = 1: N
          spd(k)= spd(k) + temp1(l,k);
       end
    end
...
function varargout = triDE_OutputFcn(hObject, eventdata, handles) 
